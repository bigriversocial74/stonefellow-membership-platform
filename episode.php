...
$pageDescription = $currentEpisode['description'] ?? 'Stonefellow episode page with watch access, member progress, and soundtrack links.';
...
<section class="sf-membership-shell sf-episode-detail-shell">
  <section class="sf-episode-hero" style="--episode-hero:url('<?= sf_asset($episodeVideo['hero'] ?? $currentEpisode['image']) ?>')">
    <div class="sf-episode-hero-copy">
      ...
      <div class="sf-episode-meta-row">
        <span><?= htmlspecialchars($episodeVideo['runtime'] ?? $currentEpisode['runtime'] ?? '48 min') ?></span>
        <span><?= htmlspecialchars($episodeVideo['status'] === 'published' ? 'Available now' : 'Coming soon') ?></span>
        <span><?= htmlspecialchars($member['access_label']) ?></span>
      </div>
      <div class="sf-episode-action-row">
        <?php if ($canWatch && ($episodeVideo['status'] ?? '') === 'published'): ?>
          <a class="sf-primary-action" href="<?= sf_url('watch.php?slug=' . urlencode($episodeVideo['slug'])) ?>">▶ Watch Episode</a>
        <?php else: ?>
          <a class="sf-primary-action" href="<?= sf_url('subscribe.php') ?>">Unlock Episode</a>
        <?php endif; ?>
        <a class="sf-secondary-action" href="<?= sf_url('watch.php?slug=' . urlencode($trailerVideo['slug'])) ?>">Watch Trailer</a>
      </div>
    </div>
    <div class="sf-episode-hero-card">
      <img src="<?= sf_asset($episodeVideo['poster'] ?? $currentEpisode['image']) ?>" alt="<?= htmlspecialchars($currentEpisode['title'] ?? 'Episode') ?> poster">
      <div class="sf-access-card">
        <strong><?= $canWatch ? 'Ready to watch' : 'Membership required' ?></strong>
        <span><?= $canWatch ? 'Episode tracking and resume are enabled.' : 'Subscribe to unlock full episode playback.' ?></span>
      </div>
    </div>
  </section>

  <section class="sf-member-grid sf-episode-grid">
    <article class="sf-member-panel sf-progress-panel">
      <span class="sf-panel-eyebrow">Episode Tracking</span>
      <h2>Continue from any device</h2>
      <p>Watch events are sent to the new tracking endpoint, then stored in video and episode progress tables when a member is signed in.</p>
      <div class="sf-progress-stack">
        <div><strong>Watch progress</strong><span><?= (int)($episodeVideo['resume_percent'] ?? 0) ?>%</span></div>
        <div class="sf-wide-progress"><i style="width:<?= (int)($episodeVideo['resume_percent'] ?? 0) ?>%"></i></div>
      </div>
    </article>

    <article class="sf-member-panel sf-access-panel">
      <span class="sf-panel-eyebrow">Access Rule</span>
      <h2><?= htmlspecialchars(sf_access_label($episodeVideo['access_level'] ?? 'subscriber')) ?></h2>
      <p>This page checks the member access level before linking to the full watch screen. Public trailer access remains available for guests.</p>
      <a href="<?= sf_url('member.php') ?>">Open member dashboard</a>
    </article>
  </section>

  <section class="sf-member-section">
    <div class="sf-member-section-head">
      <div><span class="sf-panel-eyebrow">Episode Videos</span><h2>Watch options</h2></div>
      <a href="<?= sf_url('episodes.php') ?>">All Episodes</a>
    </div>
    <div class="sf-video-card-grid">
      <?php foreach ([$episodeVideo, $trailerVideo] as $video): ?>
  ...
  <section class="sf-member-section">
    <div class="sf-member-section-head">
      <div><span class="sf-panel-eyebrow">Soundtrack Links</span><h2>Songs from this episode</h2></div>
      <a href="<?= sf_url('player.php') ?>">Open Player</a>
    </div>
    <div class="sf-audio-list-panel">
      <?php foreach ($relatedSongs as $song): ?>
        <a class="sf-audio-list-row" href="<?= sf_url('song.php?slug=' . urlencode($song['slug'])) ?>">
