-- Stonefellow migration 022: provider-agnostic merchant payments with Stripe Connect onboarding and live merchandise checkout.
...
ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS `merchant_id` INT DEFAULT NULL AFTER `user_id`,
  ADD COLUMN IF NOT EXISTS `payment_account_id` INT DEFAULT NULL AFTER `merchant_id`,
  ADD COLUMN IF NOT EXISTS `payment_provider` VARCHAR(80) DEFAULT NULL AFTER `payment_status`,
  ADD COLUMN IF NOT EXISTS `payment_currency` CHAR(3) NOT NULL DEFAULT 'USD' AFTER `payment_provider`,
  ADD COLUMN IF NOT EXISTS `provider_checkout_id` VARCHAR(190) DEFAULT NULL AFTER `external_payment_id`,
  ADD COLUMN IF NOT EXISTS `provider_customer_id` VARCHAR(190) DEFAULT NULL AFTER `provider_checkout_id`,
  ADD COLUMN IF NOT EXISTS `provider_charge_id` VARCHAR(190) DEFAULT NULL AFTER `provider_customer_id`,
  ADD COLUMN IF NOT EXISTS `payment_failure_code` VARCHAR(120) DEFAULT NULL AFTER `provider_charge_id`,
  ADD COLUMN IF NOT EXISTS `payment_failure_message` VARCHAR(500) DEFAULT NULL AFTER `payment_failure_code`,
  ADD COLUMN IF NOT EXISTS `paid_at` DATETIME DEFAULT NULL AFTER `payment_failure_message`,
  ADD COLUMN IF NOT EXISTS `refunded_at` DATETIME DEFAULT NULL AFTER `paid_at`,
  ADD COLUMN IF NOT EXISTS `disputed_at` DATETIME DEFAULT NULL AFTER `refunded_at`;
...
CREATE INDEX idx_orders_provider_checkout ON orders (payment_provider, provider_checkout_id);
CREATE INDEX idx_orders_provider_payment ON orders (payment_provider, external_payment_id);

ALTER TABLE orders
  ADD CONSTRAINT fk_orders_commerce_merchant FOREIGN KEY (merchant_id) REFERENCES commerce_merchants(id) ON DELETE SET NULL,
  ADD CONSTRAINT fk_orders_payment_account FOREIGN KEY (payment_account_id) REFERENCES merchant_payment_accounts(id) ON DELETE SET NULL;

ALTER TABLE payment_transactions
  ADD COLUMN IF NOT EXISTS `merchant_id` INT DEFAULT NULL AFTER `user_id`,
  ADD COLUMN IF NOT EXISTS `payment_account_id` INT DEFAULT NULL AFTER `merchant_id`,
  ADD COLUMN IF NOT EXISTS `provider_charge_id` VARCHAR(190) DEFAULT NULL AFTER `provider_payment_id`,
  ADD COLUMN IF NOT EXISTS `idempotency_key` VARCHAR(190) DEFAULT NULL AFTER `provider_charge_id`,
  ADD COLUMN IF NOT EXISTS `refunded_amount_cents` INT NOT NULL DEFAULT 0 AFTER `amount_cents`;

ALTER TABLE payment_transactions
  MODIFY COLUMN transaction_type ENUM('subscription','merch_order','refund','dispute','adjustment') NOT NULL DEFAULT 'subscription';

CREATE UNIQUE INDEX unique_payment_transaction_idempotency ON payment_transactions (provider, idempotency_key);
CREATE INDEX idx_payment_transactions_order_status ON payment_transactions (order_id, status, created_at);

ALTER TABLE payment_transactions
  ADD CONSTRAINT fk_payment_transactions_merchant FOREIGN KEY (merchant_id) REFERENCES commerce_merchants(id) ON DELETE SET NULL,
  ADD CONSTRAINT fk_payment_transactions_account FOREIGN KEY (payment_account_id) REFERENCES merchant_payment_accounts(id) ON DELETE SET NULL;
