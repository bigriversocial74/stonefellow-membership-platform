...
if (!sf_payment_verify_webhook($provider, $raw, $_SERVER)) {
...
if ($pdo && sf_settings_table_exists('payment_gateway_webhook_events')) {
    try {
        ...
        error_log('Stonefellow gateway duplicate check failed: ' . $e->getMessage());
    }
}

sf_payment_record_gateway_event($provider, $eventType, $payload, 'received');
$commerce = sf_commerce_process_gateway_event($provider, $eventType, $payload);
$result = !empty($commerce['handled']) ? $commerce : sf_payment_process_gateway_event($provider, $eventType, $payload);
$status = !empty($result['ignored']) ? 'ignored' : (!empty($result['ok']) ? 'processed' : 'failed');
$error = !empty($result['ok']) ? null : (string)($result['message'] ?? $result['error'] ?? 'Processing failed.');
