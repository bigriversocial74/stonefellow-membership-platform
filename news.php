...
$stories = [
  ...
  ['category'=>'Episode Guide','title'=>'Inside “Fine Print”','summary'=>'A clause nobody read becomes the most valuable character in the room.','asset'=>'episode_fine_print'],
...
<section class="lk-shell lk-news-grid">
  <?php foreach ($stories as $story): ?><article><img src="<?= lk_asset_url($story['asset']) ?>" alt="<?= htmlspecialchars($story['title'], ENT_QUOTES, 'UTF-8') ?>" loading="lazy" decoding="async"><div><p class="lk-label"><?= htmlspecialchars($story['category'], ENT_QUOTES, 'UTF-8') ?></p><h2><?= htmlspecialchars($story['title'], ENT_QUOTES, 'UTF-8') ?></h2><p><?= htmlspecialchars($story['summary'], ENT_QUOTES, 'UTF-8') ?></p><a href="<?= sf_url('signup.php') ?>">Read More →</a></div></article><?php endforeach; ?>
</section>
