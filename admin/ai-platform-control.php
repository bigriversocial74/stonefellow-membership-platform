...
function sf_aipc_execution_statuses(): array { return ['not_ready'=>'Not Ready','ready'=>'Ready','executed'=>'Executed','failed'=>'Failed','cancelled'=>'Cancelled']; }
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && ($_POST['action'] ?? '') === 'create_platform_proposal') {
  ...
  $title = sf_aipc_text($_POST['title'] ?? '', 'AI Platform Recommendation');
  $description = sf_aipc_text($_POST['description'] ?? '', 'Review this proposed platform management action before execution.');
  ...
  if (!sf_aipc_ready()) {
    ...
    $payload = ['source'=>'ai_platform_control','requested_at'=>date('c'),'suggested_next_step'=>'Review, approve, then route through the execution layer when Phase 14 is available.'];
    ...
    if ($ok) sf_admin_audit('ai_platform_action_proposed', 'ai_platform_action', null, null, ['area'=>$area,'type'=>$type,'risk'=>$risk]);
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && ($_POST['action'] ?? '') === 'update_platform_action') {
  ...
  if (!sf_aipc_ready() || $id <= 0 || !array_key_exists($approval, sf_aipc_statuses())) {
    ...
    $execution = $approval === 'ready_for_execution' ? 'ready' : (string)($before['execution_status'] ?? 'not_ready');
...
<section class="sf-ai-hero"><div class="sf-ai-panel" style="grid-column:span 3"><span class="sf-panel-eyebrow">Phase 13</span><h2>Supervised AI control</h2><p>One registry for AI-proposed management actions across story, media, publishing, membership, commerce, and operations.</p></div><div class="sf-ai-panel"><span class="sf-panel-eyebrow">Permission Model</span><h2>Review first</h2><p>AI can propose and prepare actions here. Execution waits for Phase 14 routing and approval gates.</p></div></section>
