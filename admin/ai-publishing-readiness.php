...
function sf_aipr_existing_action_keys(): array {
  ...
  $rows = sf_admin_fetch_all("SELECT action_type, target_table, target_id, approval_status FROM ai_platform_actions WHERE approval_status IN ('draft','proposed','approved','ready_for_execution') ORDER BY id DESC LIMIT 400");
  foreach ($rows as $row) $keys[(string)($row['action_type'] ?? '') . '|' . (string)($row['target_table'] ?? '') . '|' . (int)($row['target_id'] ?? 0)] = true;
  return $keys;
}
