...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && ($_POST['action'] ?? '') === 'seed_defaults') {
  if (!sf_ai_policy_ready()) {
    ...
    $count = 0;
    foreach (sf_ai_exec_routes() as $key => $route) { if (sf_ai_policy_upsert(sf_ai_policy_default_for_route($key, $route))) $count++; }
    sf_admin_flash('success', 'Default autonomy policies saved for ' . $count . ' route(s).');
  }
...
<section class="sf-admin-panel"><div class="sf-admin-panel-head"><div><span class="sf-panel-eyebrow">Defaults</span><h2>Seed route policies</h2></div><div><a href="<?= sf_url('admin/ai-platform-control.php') ?>">AI Control Center</a> · <a href="<?= sf_url('admin/ai-execution-router.php') ?>">Execution Router</a></div></div><p class="sf-pol-copy">Create or refresh one policy record for each allowlisted route. Existing routes are updated with safe defaults unless you edit them afterward.</p><form method="post" class="sf-admin-form"><?= sf_csrf_field() ?><input type="hidden" name="action" value="seed_defaults"><div class="sf-admin-form-actions"><button type="submit"<?= sf_ai_policy_ready() ? '' : ' disabled' ?>>Seed Default Policies</button></div></form></section>
