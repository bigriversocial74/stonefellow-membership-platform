...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && ($_POST['action'] ?? '') === 'save_draft_campaign') {
  if (!function_exists('sf_msg_ready') || !sf_msg_ready()) {
    sf_admin_flash('error', 'Messaging tables are not ready. Run the messaging migration before saving draft campaigns.');
    sf_admin_redirect(sf_url('admin/ai-script-team-briefing.php?season_id=' . (int)$seasonId . '&episode_id=' . (int)$episodeId . '&scene_id=' . (int)$sceneId));
  }
  $campaignId = sf_msg_save_campaign([
    'title' => 'AI Script Producer — ' . sf_aisb_text($selectedEpisode['title'] ?? '', 'Team Briefing'),
    ...
    'channel_email' => 0,
    'channel_in_app' => 1,
    'honors_preferences' => 1,
    'action_url' => sf_url('admin/ai-script-team-briefing.php?season_id=' . (int)$seasonId . '&episode_id=' . (int)$episodeId . '&scene_id=' . (int)$sceneId),
  ], 0);
  sf_admin_flash($campaignId ? 'success' : 'error', $campaignId ? 'Draft message campaign saved. Review it before queuing or sending.' : 'Draft campaign was not saved.');
  sf_admin_redirect($campaignId ? sf_url('admin/member-messaging.php?campaign_id=' . (int)$campaignId) : sf_url('admin/ai-script-team-briefing.php?season_id=' . (int)$seasonId . '&episode_id=' . (int)$episodeId . '&scene_id=' . (int)$sceneId));
}
...
<section class="sf-brief-hero">
  <div class="sf-brief-panel"><span class="sf-panel-eyebrow">Phase 6</span><h2>Draft campaign save</h2><p>Turn the selected season, episode, scene, and producer prompt into a reviewable task list and optional draft message campaign.</p></div>
  <div class="sf-brief-panel"><span class="sf-panel-eyebrow">Guardrail</span><h2>No send</h2><p>This page can save a draft campaign only. It does not queue recipients, send messages, publish content, or assign work automatically.</p></div>
</section>
...
<section class="sf-brief-layout"><article class="sf-admin-panel"><div class="sf-admin-panel-head"><div><span class="sf-panel-eyebrow">Notification Draft</span><h2><?= sf_admin_h($brief['subject']) ?></h2></div><a href="<?= sf_url('admin/member-messaging.php') ?>">Open Messaging</a></div><textarea readonly rows="14"><?= sf_admin_h($brief['body']) ?></textarea><form method="post" class="sf-admin-form"><?= sf_csrf_field() ?><input type="hidden" name="action" value="save_draft_campaign"><input type="hidden" name="season_id" value="<?= (int)$seasonId ?>"><input type="hidden" name="episode_id" value="<?= (int)$episodeId ?>"><input type="hidden" name="scene_id" value="<?= (int)$sceneId ?>"><input type="hidden" name="brief_prompt" value="<?= sf_admin_h($prompt) ?>"><div class="sf-admin-form-actions"><button type="submit"<?= (function_exists('sf_msg_ready') && sf_msg_ready()) ? '' : ' disabled' ?>>Save as Draft Campaign</button></div></form><p class="sf-brief-copy">This saves a draft only. Review it in Member Messaging before queuing recipients or sending.</p></article><aside class="sf-admin-panel"><div class="sf-admin-panel-head"><div><span class="sf-panel-eyebrow">Next Automation Step</span><h2>Human approval first</h2></div></div><div class="sf-brief-task"><strong>Draft → Review → Send</strong><p class="sf-brief-copy">The AI can prepare and save the draft campaign. Sending stays locked in Member Messaging.</p></div><div class="sf-brief-task"><strong>Future controls</strong><p class="sf-brief-copy">Next phases can add internal team roles, assignment queues, audit review, and delivery approval.</p></div></aside></section>
