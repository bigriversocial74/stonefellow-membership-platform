...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
  ...
  if (!sf_admin_db_ready() || !sf_admin_table_exists('seasons')) { sf_admin_flash('warning', 'Run migration 009 before saving seasons.'); sf_admin_redirect(); }
  $action = $_POST['action'] ?? '';
  ...
  if ($action === 'save_season') {
    ...
    $payload = ['season_number'=>sf_admin_int($_POST['season_number'] ?? null, 1) ?? 1,'title'=>$title,'slug'=>trim((string)($_POST['slug'] ?? '')) ?: sf_admin_slugify($title),'description'=>sf_admin_nullable_string($_POST['description'] ?? ''),'release_year'=>sf_admin_int($_POST['release_year'] ?? null),'status'=>$_POST['status'] ?? 'draft','poster_asset_id'=>sf_admin_int($_POST['poster_asset_id'] ?? null)];
  ...
  if ($action === 'delete_season' && $id > 0) {
    ...
    sf_admin_redirect(sf_url('admin/seasons.php'));
  }
  sf_admin_redirect();
}
...
$showSeasonForm = (isset($_GET['new']) && !$edit) || !empty($edit);
...
?>
<?php if (!$showSeasonForm): ?>
<section class="sf-admin-panel">
  <div class="sf-admin-panel-head"><div><span class="sf-panel-eyebrow">Seasons</span><h2><?= count($seasons) ?> seasons</h2></div><a href="<?= sf_url('admin/seasons.php?new=1') ?>">Add Season</a></div>
  <div class="sf-admin-table-wrap"><table class="sf-admin-table"><thead><tr><th>Season</th><th>Number</th><th>Year</th><th>Status</th><th></th></tr></thead><tbody>
    <?php if (!$seasons): ?><tr><td colspan="5">No season records yet.</td></tr><?php endif; ?>
    <?php foreach ($seasons as $season): ?><tr><td><strong><?= sf_admin_h($season['title'] ?? '') ?></strong><small><?= sf_admin_h($season['slug'] ?? '') ?></small></td><td><?= sf_admin_h($season['season_number'] ?? '—') ?></td><td><?= sf_admin_h($season['release_year'] ?? '—') ?></td><td><?= sf_admin_status_badge((string)($season['status'] ?? 'draft')) ?></td><td><a href="<?= sf_url('admin/seasons.php?edit=' . (int)($season['id'] ?? 0)) ?>">Edit</a></td></tr><?php endforeach; ?>
  </tbody></table></div>
</section>
<?php else: ?>
<section class="sf-admin-panel">
  <div class="sf-admin-panel-head"><div><span class="sf-panel-eyebrow"><?= $edit ? 'Edit Season' : 'Add Season' ?></span><h2><?= $edit ? sf_admin_h($edit['title'] ?? '') : 'New season' ?></h2></div><a href="<?= sf_url('admin/seasons.php') ?>">Back to Seasons</a></div>
  <form class="sf-admin-form" method="post">
    ...
    <label>Poster Image<?= sf_admin_asset_select('poster_asset_id', $assets, $edit['poster_asset_id'] ?? '', 'image') ?></label><?= sf_admin_asset_preview_by_id($edit['poster_asset_id'] ?? null, 'image') ?>
    <label>Status<?= sf_admin_select('status', ['draft'=>'Draft','published'=>'Published','archived'=>'Archived'], $edit['status'] ?? 'draft') ?></label>
    <div class="sf-admin-form-actions"><button type="submit"<?= sf_admin_form_disabled_attr() ?>><?= $edit ? 'Save Season' : 'Create Season' ?></button></div>
  </form>
