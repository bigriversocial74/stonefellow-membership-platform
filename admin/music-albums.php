...
if ((($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST')) {
  ...
  if ($action === 'save_album') {
    ...
    if ($id > 0) {
      ...
      sf_admin_redirect(sf_url('admin/music-albums.php?edit=' . $id));
    } else {
      ...
      sf_admin_redirect(sf_url('admin/music-albums.php?edit=' . $newId));
    }
  }
  if ($action === 'delete_album' && $id > 0) {
    ...
    sf_admin_redirect(sf_url('admin/music-albums.php'));
  }
  sf_admin_redirect();
}
...
$showAlbumForm = $isCreating || !empty($edit);
...
<section class="sf-admin-panel">
  ...
  <div class="sf-admin-table-wrap">
    <table class="sf-admin-table">
      ...
      <tbody>
        <?php if (!$albums): ?><tr><td colspan="5">No albums yet. Use Add Album to create the first album.</td></tr><?php endif; ?>
...
<section class="sf-admin-panel">
  <div class="sf-admin-panel-head">
    <div><span class="sf-panel-eyebrow"><?= $edit ? 'Edit Album' : 'Add Album' ?></span><h2><?= $edit ? sf_admin_h($edit['title'] ?? '') : 'New album' ?></h2></div>
    <a href="<?= sf_url('admin/music-albums.php') ?>">Back to Albums</a>
  </div>
