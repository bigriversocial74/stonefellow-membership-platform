...
$pageDescription = 'Post-install Stonefellow launch checklist, smoke-test matrix, and final production QA pass.';
...
sf_admin_shell_start('Launch Checklist', 'Final install QA pass', 'Use this post-install checklist after upload, SQL install, admin creation, content import, payment setup, media protection, and smoke testing.', 'launch-checklist');
?>
<section class="sf-admin-card-grid">
  ...
  <div class="sf-admin-action-card"><span>Smoke Score</span><strong><?= (int)$smokeScore ?>%</strong><small><?= (int)$smokeCounts['fail'] ?> scenario failures.</small></div>
  ...
  <div class="sf-admin-action-card"><span>Review Items</span><strong><?= (int)($reviews + $smokeCounts['manual'] + $smokeCounts['warn']) ?></strong><small>Manual/preview/warning items.</small></div>
  <a class="sf-admin-action-card" href="<?= sf_url('admin/smoke-tests.php') ?>"><span>Smoke</span><strong>Tests</strong><small>Scenario matrix and launch checks.</small></a>
...
<section class="sf-admin-panel">
  ...
  <div class="sf-admin-roadmap">
    ...
    <?php endforeach; ?>
    <div><span><?= (int)(count($launchItems) + 1) ?></span><strong>Run smoke tests</strong><p>Open the smoke-test matrix and complete auth, member, media, commerce, admin, API, backup, release, and manual provider checks.</p><a href="<?= sf_url('admin/smoke-tests.php') ?>">Smoke Tests</a></div>
  </div>
</section>
<section class="sf-admin-panel">
