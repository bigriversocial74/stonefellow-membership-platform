...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
  ...
  if ($action === 'delete_episode' && $id > 0) {
    ...
    sf_admin_redirect(sf_url('admin/episodes.php'));
  }
  sf_admin_redirect();
}
...
$showEpisodeForm = $isCreating || !empty($edit);
...
?>
<?php if (!$showEpisodeForm): ?>
<section class="sf-admin-panel">
  <div class="sf-admin-panel-head">
    <div><span class="sf-panel-eyebrow">Episodes</span><h2><?= count($episodesList) ?> episodes</h2></div>
    <a href="<?= sf_url('admin/episodes.php?new=1') ?>">Add Episode</a>
  </div>
  <div class="sf-admin-table-wrap">
    <table class="sf-admin-table">
      ...
      <tbody>
      <?php if (!$episodesList): ?><tr><td colspan="5">No episodes yet. Use Add Episode to create the first episode.</td></tr><?php endif; ?>
...
<section class="sf-admin-panel">
  <div class="sf-admin-panel-head">
    <div><span class="sf-panel-eyebrow"><?= $edit ? 'Edit Episode' : 'Add Episode' ?></span><h2><?= $edit ? sf_admin_h($edit['title'] ?? '') : 'New episode' ?></h2></div>
    <a href="<?= sf_url('admin/episodes.php') ?>">Back to Episodes</a>
  </div>
