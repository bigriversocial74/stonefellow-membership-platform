...
<section class="sf-router-hero"><div class="sf-router-panel" style="grid-column:span 3"><span class="sf-panel-eyebrow">Phase 14</span><h2>Execution router</h2><p>Approved AI platform actions are executed only through explicit, allowlisted, audited routes.</p></div><div class="sf-router-panel"><span class="sf-panel-eyebrow">Guardrail</span><h2>Allowlist only</h2><p>Unknown routes, unapproved actions, and critical-risk side effects are blocked.</p></div></section>
...
<section class="sf-admin-panel"><div class="sf-admin-panel-head"><div><span class="sf-panel-eyebrow">Allowlisted Routes</span><h2><?= count($routes) ?> route(s)</h2></div><div><a href="<?= sf_url('admin/ai-platform-control.php') ?>">AI Control Center</a></div></div><div class="sf-router-grid"><?php foreach ($routes as $key => $route): ?><article class="sf-router-card"><span class="sf-router-pill"><?= sf_admin_h($key) ?> · <?= sf_admin_h($route['side_effect']) ?></span><h3><?= sf_admin_h($route['label']) ?></h3><p class="sf-router-copy"><?= sf_admin_h($route['description']) ?></p></article><?php endforeach; ?></div></section>
