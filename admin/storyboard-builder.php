...
<section class="sf-admin-panel">
  ...
  <div class="sf-storyboard-character-grid">
    <?php foreach ($characters as $character): ?>
      <article class="sf-storyboard-character-card"><div class="sf-storyboard-thumb sf-storyboard-character-thumb" style="background-image:url('<?= sf_storyboard_h(sf_url($character['image'])) ?>')"></div><div class="sf-storyboard-character-body"><span><?= sf_storyboard_h($character['role']) ?></span><strong><?= sf_storyboard_h($character['name']) ?></strong><small><?= sf_storyboard_h($character['summary']) ?></small><small><strong>Consistency:</strong> <?= sf_storyboard_h($character['notes']) ?></small><div class="sf-admin-form-actions"><button type="button"<?= sf_admin_form_disabled_attr() ?>>View Details</button><button type="button"<?= sf_admin_form_disabled_attr() ?>>Replace Image</button></div></div></article>
    <?php endforeach; ?>
...
<section class="sf-admin-panel">
  ...
  <div class="sf-storyboard-scene-grid">
    <?php foreach ($scenes as $scene): $sceneId = (int)($scene['id'] ?? 0); $sceneDisabled = ($sceneId > 0 && sf_storyboard_ready()) ? '' : ' disabled'; $currentSceneStatus = strtolower(str_replace(' ', '_', (string)($scene['status'] ?? 'draft'))); ?>
      <article id="scene-<?= (int)$scene['number'] ?>" class="sf-storyboard-scene-card">
        ...
        <div class="sf-storyboard-scene-body">
          ...
          <details class="sf-storyboard-scene-editor"><summary>Edit Scene</summary>
            <form class="sf-admin-form" method="post" action="<?= sf_url('api/storyboard-scene-action.php') ?>">
              <?= sf_csrf_field() ?><input type="hidden" name="action" value="edit_scene"><input type="hidden" name="scene_id" value="<?= $sceneId ?>"><input type="hidden" name="return_url" value="<?= sf_storyboard_h($returnUrl . '#scene-' . (int)$scene['number']) ?>">
...
<section class="sf-admin-panel"><div class="sf-admin-panel-head"><div><span class="sf-panel-eyebrow">Implementation Notes</span><h2>What Phase 43 adds</h2></div><a href="<?= sf_url('docs/PHASE_43_SCENE_ACTIONS.md') ?>">Phase Docs</a></div><div class="sf-admin-roadmap"><div><span>Edit</span><strong>Scene persistence</strong><p>Scene title, prompt, image prompt, dialog, notes, location, time, and status can be saved.</p></div><div><span>AI</span><strong>Rewrite scene</strong><p>One scene can be rewritten with continuity and character consistency context.</p></div><div><span>Image</span><strong>Regenerate or upload</strong><p>Scene image regeneration uses the image provider; uploads replace the active scene image.</p></div><div><span>Jobs</span><strong>Retry-ready tracking</strong><p>Scene actions create job records with success/failure output for retries.</p></div></div></section>
