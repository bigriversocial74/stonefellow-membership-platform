...
$allChecks=array_merge($qaChecks,$pkgChecks,$smokeChecks,$rcChecks,$opsChecks);
$qaScore=sf_qa_score($qaChecks);$pkgScore=sf_pkg_score($pkgChecks);$smokeScore=sf_smoke_score($smokeChecks);$rcScore=sf_rc_score($rcChecks);$opsScore=sf_dor_score($opsChecks);$overallScore=sf_qa_score($allChecks);
...
echo "\nLaunch Gate\n-----------\n";echo count($fails)>0?"BLOCKED: Resolve all code, data, backup, release, and recovery failures before launch.\n":"READY: No blocking failures detected. Complete remaining manual checks before launch.\n";
