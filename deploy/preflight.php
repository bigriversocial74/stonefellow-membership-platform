...
echo "\nSmoke Tests\n";
echo "-----------\n";
foreach (sf_smoke_group_summary($smokeChecks) as $group) {
  echo $group['group'] . ': ' . (int)$group['score'] . '% · scenarios ' . (int)$group['count'] . ' · fails ' . (int)$group['fails'] . ' · warnings ' . (int)$group['warnings'] . ' · manual ' . (int)$group['manual'] . "\n";
}
if ($fails) {
  echo "\nBlocking Failures\n";
  echo "-----------------\n";
