...
function sf_rc_target_migration(): string { return '020'; }
function sf_rc_zip_url(): string { return 'https://github.com/' . sf_rc_repo_full_name() . '/archive/refs/heads/' . sf_rc_target_branch() . '.zip'; }
function sf_rc_check(string $group, string $status, string $label, string $detail = '', int $weight = 1, string $url = ''): array {
  return ['group'=>$group,'status'=>$status,'label'=>$label,'detail'=>$detail,'weight'=>max(1,$weight),'url'=>$url];
}
...
function sf_rc_release_tasks_summary(?array $release = null): array {
  ...
  return [
    'total'=>count($tasks),
    'passed'=>count(array_filter($tasks, static fn($t) => ($t['status'] ?? '') === 'passed')),
    'failed'=>count(array_filter($tasks, static fn($t) => ($t['status'] ?? '') === 'failed')),
    'waived'=>count(array_filter($tasks, static fn($t) => ($t['status'] ?? '') === 'waived')),
    'pending'=>count(array_filter($tasks, static fn($t) => ($t['status'] ?? 'pending') === 'pending')),
  ];
}
...
function sf_rc_counts(array $checks): array {
  return [
    'total'=>count($checks),
...
function sf_rc_status_text(array $checks): string {
  $counts = sf_rc_counts($checks);
  $score = sf_rc_score($checks);
  if ((int)$counts['fail'] > 0) return 'blocked';
  if ((int)$counts['warn'] > 0 || (int)$counts['manual'] > 0) return 'manual_review';
  return $score >= 97 ? 'release_candidate' : 'needs_review';
}
function sf_rc_summary(): array {
  ...
  return [
    'repo'=>sf_rc_repo_full_name(),
    'branch'=>sf_rc_target_branch(),
    'zip_url'=>sf_rc_zip_url(),
    'migration_target'=>sf_rc_target_migration(),
    'qa_score'=>sf_qa_score($qaChecks),
    'package_score'=>sf_pkg_score($pkgChecks),
    'smoke_score'=>sf_smoke_score($smokeChecks),
    'release_candidate_score'=>sf_rc_score($rcChecks),
    'release_candidate_status'=>sf_rc_status_text($rcChecks),
    'counts'=>sf_rc_counts($rcChecks),
  ];
}
function sf_rc_render_table(array $checks): void {
  ...
  foreach ($checks as $check) {
    ...
    echo '<tr><td>' . sf_rc_h($check['group'] ?? '') . '</td><td><strong>' . sf_rc_h($check['label'] ?? '') . '</strong></td><td>' . sf_qa_badge((string)($check['status'] ?? 'info')) . '</td><td>' . sf_rc_h($check['detail'] ?? '') . '</td><td>' . ($url ? '<a href="' . sf_rc_h(strpos($url, 'http') === 0 ? $url : sf_url($url)) . '">Open</a>' : '—') . '</td></tr>';
  }
