...
if (is_file($localConfigPath)) {
    ...
    if (is_array($localConfig)) {
        ...
        if (!empty($localConfig['security']) && is_array($localConfig['security'])) {
            $security = array_merge($security, $localConfig['security']);
        }
    }
}

// Keep legacy helpers that read SF_HASH_SALT compatible with installer-generated app keys.
if (!getenv('SF_APP_KEY') && !empty($security['app_key'])) {
    putenv('SF_APP_KEY=' . (string)$security['app_key']);
}
if (!getenv('SF_HASH_SALT')) {
    $configuredSalt = trim((string)($security['hash_salt'] ?? $security['app_key'] ?? ''));
    if ($configuredSalt !== '') {
        putenv('SF_HASH_SALT=' . $configuredSalt);
    }
}
...
function sf_base_path(): string
{
    ...
    if (in_array($last, ['admin', 'api', 'deploy'], true)) {
...
function sf_absolute_url(string $path = ''): string
{
    ...
    $scheme = sf_is_https() ? 'https' : 'http';
    return $scheme . '://' . sf_security_request_host() . '/' . ltrim($url, '/');
}
...
function sf_asset(string $path): string
{
    ...
    if (!str_starts_with(strtolower($cleanPath), 'assets/')) {
...
{
    if (PHP_SAPI === 'cli' || sf_env_bool('SF_SKIP_INSTALL_REDIRECT', false)) {
        return;
    }

    $script = basename((string)($_SERVER['SCRIPT_NAME'] ?? ''));
    if ($script === 'install.php' || sf_is_installed()) {
        return;
    }
