...
$sfPublicNav = [
  ...
  ['label' => 'Extras', 'href' => 'extras.php', 'pages' => ['extras.php']],
  ['label' => 'News', 'href' => 'news.php', 'pages' => ['news.php']],
];
$sfCurrentPage = sf_current_page();
$sfMainNav = $sfHeaderUser ? [] : $sfPublicNav;
$sfBodyClass = trim($sfPageClass . ($sfHeaderUser ? ' sf-logged-in' : '') . ($sfIsCustomerSurface ? ' sf-customer-ui-page' : '') . (!$sfIsAdminSurface ? ' likenessing-theme' : ''));
...
$sfSocialImage = $sfIsAdminSurface ? sf_frontend_social_image() : sf_asset('images/likenessing/hero-ensemble-v2.webp');
...
<head>
  ...
  <link rel="apple-touch-icon" href="<?= $sfIsAdminSurface ? sf_asset('images/brand/logo-mark.png') : sf_asset('images/likenessing/favicon-v2.png') ?>">
...
<body class="<?= htmlspecialchars($sfBodyClass, ENT_QUOTES, 'UTF-8') ?>">
  ...
  <header class="home-header-full site-global-header">
    <div class="home-header">
      <a class="home-brand" href="<?= sf_url('index.php') ?>" aria-label="<?= htmlspecialchars($sfSiteName, ENT_QUOTES, 'UTF-8') ?> home"><img src="<?= $sfIsAdminSurface ? sf_asset('images/brand/home-brand-approved.png') : sf_asset('images/likenessing/logo-lockup-v2.png') ?>" alt="<?= htmlspecialchars($sfSiteName, ENT_QUOTES, 'UTF-8') ?>" class="home-brand-image" decoding="async" fetchpriority="high"></a>
      ...
      <div class="home-header-actions">
        <?php if ($sfHeaderUser): ?>
          <details class="home-user-menu"><summary class="home-user-summary"><span class="lk-icon lk-icon-account" aria-hidden="true"></span><span><?= htmlspecialchars($sfHeaderUser['display_name'] ?: 'Account', ENT_QUOTES, 'UTF-8') ?></span></summary><div class="home-user-dropdown"><a href="<?= sf_url('member.php') ?>">Member Dashboard</a><a href="<?= sf_url('library.php') ?>">My Library</a><a href="<?= sf_url('watchlist.php') ?>">Watchlist</a><a href="<?= sf_url('notifications.php') ?>">Notifications</a><a href="<?= sf_url('messages.php') ?>">Messages</a><a href="<?= sf_url('comments.php') ?>">Comments</a><a href="<?= sf_url('cart.php') ?>">Cart</a><a href="<?= sf_url('account.php') ?>">Account Settings</a><a href="<?= sf_url('account-billing.php') ?>">Billing</a><a href="<?= sf_url('support.php') ?>">Support</a><?php if ($sfIsAdminUser): ?><a class="home-user-admin-link" href="<?= sf_url('admin/index.php') ?>">Admin Dashboard</a><?php endif; ?><a href="<?= sf_url('logout.php') ?>">Logout</a></div></details>
        <?php else: ?>
          <details class="home-user-menu home-user-menu-public"><summary class="home-user-summary"><span class="lk-icon lk-icon-account" aria-hidden="true"></span><span>Account</span></summary><div class="home-user-dropdown"><a href="<?= sf_url('signin.php') ?>">Sign In</a><a href="<?= sf_url('signup.php') ?>">Create Account</a><a href="<?= sf_url('forgot-password.php') ?>">Forgot Password</a></div></details><a href="<?= sf_url('episodes.php') ?>" class="home-subscribe-btn <?= sf_is_active('episodes.php') ?>">Watch Now</a>
        <?php endif; ?>
      </div>
    </div>
  </header>
