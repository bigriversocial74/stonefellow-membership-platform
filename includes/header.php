...
<body class="<?= htmlspecialchars($pageClass ?? '') ?>">
  ...
  <header class="home-header-full site-global-header">
    <div class="home-header">
      <a class="home-brand" href="<?= sf_url('index.php') ?>" aria-label="Stonefellow home">
        <img src="<?= sf_asset('images/brand/home-brand-approved.png') ?>" alt="Stonefellow" class="home-brand-image">
      </a>
      <button class="nav-toggle home-nav-toggle" type="button" aria-label="Open navigation" data-nav-toggle>
      ...
      <nav class="home-nav" data-site-nav>
        ...
        <?php $memberNavActive = in_array(sf_current_page(), ['member.php', 'library.php', 'watchlist.php', 'playlists.php', 'account.php', 'account-billing.php', 'billing-checkout.php', 'billing-success.php', 'billing-cancel.php'], true) ? 'is-active' : ''; ?>
      ...
      <div class="home-header-actions">
        <?php if ($sfHeaderUser): ?>
          <a href="<?= sf_url('account.php') ?>" class="home-link-btn <?= sf_is_active('account.php') ?>"><?= htmlspecialchars($sfHeaderUser['display_name'] ?: 'Account') ?></a>
          <a href="<?= sf_url('logout.php') ?>" class="home-subscribe-btn">Logout</a>
        <?php else: ?>
          <a href="<?= sf_url('signin.php') ?>" class="home-link-btn <?= sf_is_active('signin.php') ?>">Sign In</a>
  ...
  <main>
    ...
    <?php if ($sfAuthFlashes): ?>
      <div class="sf-flash-stack" role="status" aria-live="polite">
        <?php foreach ($sfAuthFlashes as $flash): ?>
          <div class="sf-flash sf-flash-<?= htmlspecialchars($flash['type'] ?? 'info') ?>"><?= htmlspecialchars($flash['message'] ?? '') ?></div>
        <?php endforeach; ?>
      </div>
    <?php endif; ?>
