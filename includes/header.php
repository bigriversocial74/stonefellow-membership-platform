...
require_once __DIR__ . '/likenessing_theme.php';
...
$sfPublicNav = [
  ...
  ['label' => 'Extras', 'href' => 'extras.php', 'pages' => ['extras.php', 'music.php', 'player.php', 'album.php', 'song.php']],
  ['label' => 'News', 'href' => 'news.php', 'pages' => ['news.php']],
...
$sfThemeClass = $sfIsAdminSurface ? '' : ' likenessing-theme';
$sfBodyClass = trim($sfPageClass . $sfThemeClass . ($sfHeaderUser ? ' sf-logged-in' : '') . ($sfIsCustomerSurface ? ' sf-customer-ui-page' : ''));
$sfSiteName = $sfIsAdminSurface ? (string)($site['name'] ?? 'Stonefellow') : 'Likenessing';
...
$sfMetaDescription = trim((string)($pageDescription ?? ($sfIsAdminSurface ? ($site['tagline'] ?? '') : 'Fame. Fortune. Full control. Likenessing is an original dark comedy about identity, artificial intelligence, and the contracts that own both.')));
$sfCanonical = sf_frontend_canonical_url();
$sfSocialImage = $sfIsAdminSurface ? sf_frontend_social_image() : lk_asset_url('hero');
...
<head>
  ...
  <meta name="theme-color" content="<?= $sfIsAdminSurface ? '#0b0907' : '#050505' ?>">
  <meta name="color-scheme" content="dark">
  <meta name="apple-mobile-web-app-capable" content="yes">
  <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
  <meta name="apple-mobile-web-app-title" content="<?= $sfIsAdminSurface ? 'Stonefellow' : 'Likenessing' ?>">
  ...
  <link rel="apple-touch-icon" href="<?= $sfIsAdminSurface ? sf_asset('images/brand/logo-mark.png') : lk_asset_url('favicon') ?>">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Playfair+Display:wght@500;600&family=Bebas+Neue&family=Cinzel:wght@600;700&family=Bodoni+Moda:opsz,wght@6..96,500;6..96,600;6..96,700&display=swap" rel="stylesheet">
  ...
  <?php if (!$sfIsAdminSurface): ?><link rel="stylesheet" href="<?= sf_asset('css/likenessing.css?v=20260716') ?>"><?php endif; ?>
...
<body class="<?= htmlspecialchars($sfBodyClass, ENT_QUOTES, 'UTF-8') ?>">
  ...
  <header class="home-header-full site-global-header">
    <div class="home-header">
      <a class="home-brand" href="<?= sf_url('index.php') ?>" aria-label="<?= $sfIsAdminSurface ? 'Stonefellow' : 'Likenessing' ?> home">
        <img src="<?= $sfIsAdminSurface ? sf_asset('images/brand/home-brand-approved.png') : lk_asset_url('logo') ?>" alt="<?= $sfIsAdminSurface ? 'Stonefellow' : 'Likenessing — an original comedy series' ?>" class="home-brand-image" decoding="async" fetchpriority="high">
      </a>
      ...
      <div class="home-header-actions">
        <?php if ($sfHeaderUser): ?>
          <details class="home-user-menu"><summary class="home-user-summary"><span class="lk-icon lk-icon-account" aria-hidden="true"></span><span><?= htmlspecialchars($sfHeaderUser['display_name'] ?: 'Account', ENT_QUOTES, 'UTF-8') ?></span></summary><div class="home-user-dropdown"><a href="<?= sf_url('member.php') ?>">Member Dashboard</a><a href="<?= sf_url('library.php') ?>">My Library</a><a href="<?= sf_url('watchlist.php') ?>">Watchlist</a><a href="<?= sf_url('playlists.php') ?>">Playlists</a><a href="<?= sf_url('notifications.php') ?>">Notifications</a><a href="<?= sf_url('messages.php') ?>">Messages</a><a href="<?= sf_url('comments.php') ?>">Comments</a><a href="<?= sf_url('cart.php') ?>">Cart</a><a href="<?= sf_url('account.php') ?>">Account Settings</a><a href="<?= sf_url('account-billing.php') ?>">Billing</a><a href="<?= sf_url('support.php') ?>">Support</a><?php if ($sfIsAdminUser): ?><a class="home-user-admin-link" href="<?= sf_url('admin/index.php') ?>">Admin Dashboard</a><?php endif; ?><a href="<?= sf_url('logout.php') ?>">Logout</a></div></details>
        <?php else: ?>
          <details class="home-user-menu home-user-menu-public"><summary class="home-user-summary"><span class="lk-icon lk-icon-account" aria-hidden="true"></span><span>Account</span></summary><div class="home-user-dropdown"><a href="<?= sf_url('signin.php') ?>">Sign In</a><a href="<?= sf_url('signup.php') ?>">Create Account</a><a href="<?= sf_url('forgot-password.php') ?>">Forgot Password</a></div></details><a href="<?= sf_url('subscribe.php') ?>" class="home-subscribe-btn <?= sf_is_active('subscribe.php') ?>">Watch Now <span aria-hidden="true">▶</span></a>
        <?php endif; ?>
      </div>
    </div>
  </header>
