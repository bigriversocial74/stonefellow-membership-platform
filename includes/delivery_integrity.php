...
function sf_delivery_mask_email(string $email): string { $email=sf_delivery_safe_email($email);if($email==='')return '';$parts=explode('@',$email,2);$local=$parts[0];$visible=function_exists('mb_substr')?mb_substr($local,0,1):substr($local,0,1);$length=function_exists('mb_strlen')?mb_strlen($local):strlen($local);return $visible.str_repeat('*',max(2,$length-1)).'@'.$parts[1]; }
